	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(method.equals("mreg")){ //用户注册
			...
			if(username.equals("admin")){
				...
				if(str==null){
					int flag=cBean.comUp("insert into member(username,password,realname,sex,age,address,email ) " +
							"values('"+username+"','"+password+"','"+realname+"','"+sex+"','"+age+"','"+address+"','"+email+"' )");
			...
			String username = request.getParameter("username"); 
			String email = request.getParameter("email"); 
			int flag = cBean.comUp("update member set password='111' where username='"+username+"' and  email='"+email+"' and ifuse='在用'");
			if(flag == Constant.SUCCESS){ 
				request.setAttribute("message", "新密码为111，请登录修改！");
