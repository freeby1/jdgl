...
<%	String word=Common.toChineseAndTrim(request.getParameter("word")); 
	...
	int cou = cb.getMessageCount("select count(*) from dd where ddh like '%"+word+"%' and member='"+member+"' ");//得到信息总数			        
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from dd where ddh like '%"+word+"%' and member='"+member+"' order by id desc",19);
...
%>       
       <tr align="center">
         ...
         <td nowrap="nowrap">  
         <%if(pagelist2.get(16).toString().equals("已结账")){%><a href="<%=basePath%>member/dd/pj.jsp?method=pjdd&id=<%=pagelist2.get(0).toString()%>">评价</a><%} %> 
         <a href="<%=basePath%>ComServlet?method=deldd&id=<%=pagelist2.get(0).toString()%>">取消</a>
         </td>
       </tr>
<% }} %>
       <tr align="center">
