...
String message = (String)request.getAttribute("message");
	...
	else{  List rklist = cb.getCom("select * from member order by id desc",3);  
		if(!rklist.isEmpty()){
		for(int i=0;i<rklist.size();i++){
			List list2 =(ArrayList)rklist.get(i); 
			int jf=cb.getCount("select sum(jf) from dd where member='"+list2.get(1).toString()+"'");  
			cb.comUp("update member set jf='"+jf+"' where id='"+list2.get(0).toString()+"' ");
		}}
%>
<body>
...
<%String word=Common.toChineseAndTrim(request.getParameter("word")); 
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from member where username like '%"+word+"%' order by id desc",11);
...
%>       
       <tr align="center">
         ...
         <td nowrap="nowrap">
         <a href="<%=basePath%>admin/member/hy.jsp?method=hy&id=<%=pagelist2.get(0).toString()%>">会员</a>
         <a href="<%=basePath%>MemberServlet?method=ifusem&id=<%=pagelist2.get(0).toString()%>"><%=pagelist2.get(9).toString() %></a> 
         <a href="<%=basePath%>MemberServlet?method=delm&id=<%=pagelist2.get(0).toString()%>">删除</a></td>
       </tr>
<% }} %>
       <tr align="center">
