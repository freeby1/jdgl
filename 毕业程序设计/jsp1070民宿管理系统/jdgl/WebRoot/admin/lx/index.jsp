...
function top2(){
   	form3.action="<%=basePath%>admin/lx/index.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>admin/lx/index.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>admin/lx/index.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>admin/lx/index.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>admin/lx/index.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>admin/lx/index.jsp?page="+page;
      form3.submit();
...
<div class="title_right"><strong>会员信息管理</strong></div>  
...
<%   
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from lx order by id desc",3);
...
%>       
       <tr align="center">
         ...
         <td nowrap="nowrap">
         <a href="<%=basePath%>admin/lx/add.jsp?method=uplx&id=<%=pagelist2.get(0).toString()%>">修改</a> 
         <a href="<%=basePath%>ComServlet?method=dellx&id=<%=pagelist2.get(0).toString()%>">删除</a>
         </td>
       </tr>
<% }} %>
       <tr align="center">
